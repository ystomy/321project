        ...
        private void PlaySoundAndWait() {
            ...
            if (bus != null && bus.IsPaused) {
                if (ParentGroup.LoggingEnabledForGroup) {
                    MasterAudio.LogWarning("Starting Variation '" + name + "' of Sound Group '" + ParentGroup.GameObjectName + "' because its Bus is paused.");
                }

                GrpVariation.Pause();
            } else if (ParentGroup.IsPaused) { // start paused is Sound Group is paused
                if (ParentGroup.LoggingEnabledForGroup) {
                    MasterAudio.LogWarning("Starting Variation '" + name + "' of Sound Group '" + ParentGroup.GameObjectName + "' because the Sound Group is paused.");
                }
                GrpVariation.Pause();
            }            
        ...
        public void MaybeChain() {
            ...
            if (playSnd.AttachToSource || playSnd.SourceTrans != null) {
                ...
                if (chainedVariation.ActingVariation != null) {
                    // must set and inform TransformFollower of the new Variation so it doesn't continue to follow the old, for AmbientSound script.
                    chainedVariation.ActingVariation.UpdateAudioVariation(GrpVariation.AmbientFollower);

                    // must set and inform the new Variation so automation will affect the new Variation, and it won't stop the command, for RTPC trackers.
                    MasterAudio.UpdateRealTimeParameterCommandTrackersWithNewChainedVariation(GrpVariation, chainedVariation.ActingVariation);
                }
            } else {
