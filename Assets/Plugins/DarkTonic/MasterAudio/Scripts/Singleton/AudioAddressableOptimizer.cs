...
namespace DarkTonic.MasterAudio {
    ...
    public static class AudioAddressableOptimizer {
        ...
        public static IEnumerator PopulateSourceWithAddressableClipAsync(AssetReference addressable, SoundGroupVariation variation, int unusedSecondsLifespan,
            ...
            if (!AudioUtil.AudioClipIsStreaming(addressableClip) && !AudioUtil.AudioClipWillPreload(addressableClip)) {
                MasterAudio.LogWarning("Audio Clip for Addressable file '" + addressableClip.CachedName() + "' of Sound Group '" + variation.ParentGroup.GameObjectName + "' has 'Preload Audio Data' turned off, which can cause audio glitches. Addressables should always Preload Audio Data if not set to Streaming Load Type. Please turn it on.");
            }
        ...
        public static IEnumerator PopulateAddressableSongToPlaylistControllerAsync(MusicSetting setting, AssetReference addressable,
            ...
            if (!AudioUtil.AudioClipIsStreaming(addressableClip) && !AudioUtil.AudioClipWillPreload(addressableClip)) {
                MasterAudio.LogWarning("Audio Clip for Addressable file '" + addressableClip.CachedName() + "' of Playlist Controller '" + playlistController.ControllerName + "' has 'Preload Audio Data' turned off, which can cause audio glitches. Addressables should always Preload Audio Data if not set to Streaming Load Type. Please turn it on.");
            }
