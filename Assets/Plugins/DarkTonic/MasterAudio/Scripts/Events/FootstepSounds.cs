    ...
    public class FootstepSounds : MonoBehaviour {
        ...
        private void PlaySoundsIfMatch(GameObject go) {
            ...
            for (var i = 0; i < footstepGroups.Count; i++) {
                ...
                switch (aGroup.playSoundType) {
                    case EventSounds.PlaySoundType.InvokeParameterCommand:
                        if (aGroup.parameterCommandName == MasterAudio.NoGroupName) {
                            continue;
                        }

                        MasterAudio.InvokeParameterCommand(aGroup.parameterCommandName, Trans);

                        break;
                    case EventSounds.PlaySoundType.PlaySoundGroup:
                        if (aGroup.soundType == MasterAudio.NoGroupName) {
                            continue;
                        }
